...
export class ProfessorStatsService implements AbstractProfessorStatsService {
  ...
  async incrementMarathonsProfessorStats(userId: string): Promise<void> {
    await this.professorStatsRepository.incrementMarathons(userId);
  }

  async incrementTotalStudentsProfessorStats(userId: string): Promise<void> {
    await this.professorStatsRepository.incrementStudentsReached(userId);
  }
}
